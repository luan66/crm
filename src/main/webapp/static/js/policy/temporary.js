$(function () {
    ...
    var methodObject = {
        ...
        edit: function () {
            ...
            $.get("/policy/policyDetail.do", {id:temporary.id}, function (data) {
                ...
                $("#customergender").combobox("setValue",data.customer.gender);
                $("[name='customer.tel']").val(data.customer.tel);
                $("[name='customer.idNo']").val(data.customer.idNo);
                $("#customeraddress").textbox("setValue",data.customer.address);
                // 设置车辆信息
                console.debug(data.car);
                $("[name='car.id']").val(data.car.id);

                $("#carcustomername").textbox("setValue",data.customer.name);
                $("#carbrand").textbox("setValue",data.car.brand);
                $("#carmodel").textbox("setValue",data.car.model);
                $("#cargasDisplacement").textbox("setValue",data.car.gasDisplacement);
                $("#carplateNumber").textbox("setValue",data.car.plateNumber);
                $("#carcategory").combobox("setValue",data.car.category);
                $("#carvaluation").textbox("setValue",data.car.valuation);
                $("#carsize").combobox("setValue",data.car.size);
                $("#carpurchaseDate").datebox('setValue', data.car.purchaseDate);
                // 设置保险产品
        ...
        putUp: function () {
            ...
            var id = temporary.id;
            $.get("/policy/putUp.do",{id:id}, function (data) {
                if (data.success) {
        ...
        refuse: function () {
            ...
            $.messager.prompt('警告信息', '你确定要拒绝用户: '+temporary.customer.name+'的投保申请吗? <a/>如是, 请输入拒保原因：',function(r){
                $.post("/policy/refuse.do",{id:id,remark:r}, function (data) {
                    if (data.success) {
                        $.messager.alert("温馨提示", data.message, "info", function () {
                            temporary_dialog.dialog("close");
                            temporary_datagrid.datagrid("load");
    ...
    temporary_datagrid.datagrid({
        ...
        fitColumns: true,
        striped: true,
        url: "/policy/temporaryList.do?state=0",
        pagination: true,
        singleSelect: true,
        ...
        columns: [[
            ...
            {
                field: 'remark', title: '查看商品明细', width: 100, align: 'center', formatter: function (value, row, index) {
                return "<a href='#' id='detail' onclick='showAdvice();'>查看商品明细</a><input id='remark' type='hidden' value='"+value+"'>";
            }
                // {
                //     field: 'remark', title: '查看批改信息', width: 100, align: 'center'
                // }
                /*{
...
function showAdvice() {
    $.messager.alert("商品明细","数据加载中, 请稍等...","info");
}
